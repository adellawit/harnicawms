...
return new class extends Migration
{
    ...
    private const MENU_ID = 'a1c00001-f001-4001-8001-00000000000d';
    ...
    public function up(): void
    {
        ...
        $exists = DB::table('master_data.menus')
            ->where(function ($q) {
                $q->where('id', self::MENU_ID)->orWhere('code', 'general_ledger');
            })
            ->exists();
        ...
        if (! $exists && DB::table('master_data.menus')->where('id', self::FINANCE_PARENT_ID)->exists()) {
            DB::table('master_data.menus')->insert([
                ...
                'name' => 'General Ledger',
                'code' => 'general_ledger',
                'text_sidebar' => 'General Ledger',
                'icon' => 'ti ti-list-details',
                'has_page' => false,
                'url_path' => 'finance/general-ledger',
                'route_name' => 'finance.general-ledger.index.view',
                'slug' => 'general-ledger',
                'level_sidebar' => 2,
                'order_number' => 12,
                'is_label' => false,
    ...
    {
        if (Schema::hasTable('auth.iam_has_accesses')) {
            DB::table('auth.iam_has_accesses')->where('sidebar_menu_id', self::MENU_ID)->delete();
        }
        if (Schema::hasTable('master_data.menus')) {
