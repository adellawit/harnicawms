...
return new class extends Migration
{
    ...
    private const MENU_ID = 'a1c00001-f001-4001-8001-000000000004';
    ...
    public function up(): void
    {
        ...
        $exists = DB::table('master_data.menus')
            ->where(function ($q) {
                $q->where('id', self::MENU_ID)
                    ->orWhere('code', 'cash_flow_category');
            })
            ->exists();
        ...
        if (! $exists && DB::table('master_data.menus')->where('id', self::FINANCE_PARENT_ID)->exists()) {
            DB::table('master_data.menus')->insert([
                ...
                'code' => 'cash_flow_category',
                'text_sidebar' => 'Cash Flow Category',
                'icon' => 'ti ti-arrows-split',
                'has_page' => false,
                'url_path' => 'finance/cash-flow-category',
                'route_name' => 'finance.cash-flow-category.index.view',
                'slug' => 'cash-flow-category',
                'level_sidebar' => 2,
                'order_number' => 3,
                'is_label' => false,
                'has_create' => true,
