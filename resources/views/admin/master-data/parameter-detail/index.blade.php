<x-app-layout>
    ...
    @section('title', $parameter->name . ' Details | ')
    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        ...
        <x-page-header
            :breadcrumbs="[
                ...
                ['label' => 'Parameter', 'url' => route('parameter.index.view')],
                ['label' => $parameter->name, 'active' => true]
            ]"
        />
    ...
    <div class="modal fade" id="deleteModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                ...
                <form method="POST" action="{{ route('parameter.details.delete.data', $parameterId) }}">
                    @csrf
                    <div class="modal-body">
                        <div class="row">
                            <div class="col">
                                <p class="">Are you sure you want to delete <strong
                                        id="parameter-detail-code-deleted"></strong>?
                                </p>
                                <input type="text" id="parameter-detail-id-deleted" name="parameter_detail_id_deleted"
    ...
    <div class="modal fade" id="restoreModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                ...
                <form method="POST" action="{{ route('parameter.details.restore.data', $parameterId) }}">
                    @csrf
                    <div class="modal-body">
                        <div class="row">
                            <div class="col">
                                <p class="">Are you sure you want to restore <strong
                                        id="parameter-detail-code-restore"></strong>?
                                </p>
                                <input type="text" id="parameter-detail-id-restore" name="parameter_detail_id_restored"
            ...
            $(document).ready(function() {
                $('#table').DataTable({
                        ...
                        columns: [{
                            ...
                            },
                            {
                                ...
                                render: function(data, type, row, meta) {
                                    ...
                                    var btnEdit = '<li><a class="dropdown-item" href="{{ url("master-data/parameter/{$parameterId}/details/edit") }}/' + row.id + '">' +
                                        '<i class="ti ti-pencil me-2 text-warning"></i>Edit</a></li>';

                                    var btnDelete = '<li><button type="button" class="dropdown-item" data-bs-toggle="modal" data-bs-target="#deleteModal" data-id="' +
                                        row.id + '" data-code="' + (row.key || '') + '">' +
                                        '<i class="ti ti-trash me-2 text-danger"></i>Delete</button></li>';

                                    var btnRestore = '<li><button type="button" class="dropdown-item" data-bs-toggle="modal" data-bs-target="#restoreModal" data-id="' +
                                        row.id + '" data-code="' + (row.key || '') + '">' +
                        ...
                        buttons: [{
                            ...
                            text: '<i class="ti ti-plus me-sm-1"></i> <span class="d-none d-sm-inline-block">Add Detail</span>',
                            className: "create-new btn btn-primary",
                            action: function(e, dt, node, config) {
                                window.location = '{{ route("parameter.details.insert.view", $parameterId) }}';
                            },
                        }, ],
                    }),
                    $("div.head-label").html('<h4 class="card-title mb-0">{{ $parameter->name }} - Details</h4>');

                $("#btnFilter").click(function() {
                    var status = $('#selectStatus').find(':selected').val();

                    var path = "{{ route("parameter.details.index.view", $parameterId) }}?";
                ...
                $("#btnResetFilter").click(function() {
                    window.location = '{{ route("parameter.details.index.view", $parameterId) }}';
                });
                ...
                $('#deleteModal').on('show.bs.modal', function(event) {
                    ...
                    $('#parameter-detail-id-deleted').val(id);
                    $('#parameter-detail-code-deleted').text(code);
                });
                ...
                $('#restoreModal').on('show.bs.modal', function(event) {
                    ...
                    $('#parameter-detail-id-restore').val(id);
                    $('#parameter-detail-code-restore').text(code);
                });
            });
        </script>
    @endpush
