<x-app-layout>
    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        @php
            $hasUpdatePermission = session('permissions.Configuration.is_update', false) == 1;
            $hasDeletePermission = session('permissions.Configuration.is_delete', false) == 1;
            $hasCreatePermission = session('permissions.Configuration.is_create', false) == 1;
    ...
    <div class="modal fade" id="deleteModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                ...
                <form method="POST" action="{{ route('parameter.delete.data') }}">
                    @csrf
                    <div class="modal-body">
                        <div class="row">
                            <div class="col">
                                <p class="">Are you sure you want to delete <strong
                                        id="parameter-name-deleted"></strong>?
                                </p>
                                <input type="text" id="parameter-id-deleted" name="parameter_id_deleted"
    ...
    <div class="modal fade" id="restoreModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                ...
                <form method="POST" action="{{ route('parameter.restore.data') }}">
                    @csrf
                    <div class="modal-body">
                        <div class="row">
                            <div class="col">
                                <p class="">Are you sure you want to restore <strong
                                        id="parameter-name-restore"></strong>?
                                </p>
                                <input type="text" id="parameter-id-restore" name="parameter_id_restored"
        ...
        <script>
            $(document).ready(function() {
                $('#table').DataTable({
                        ...
                        scrollX: true,
                        ajax: {
                            "url": "{{ route('parameter.index.data') }}",
                            "type": "POST",
                            "data": {
                                _token: "{{ csrf_token() }}",
                                "status": "{{ $status }}",
                        ...
                        columns: [{
                            ...
                            @if($hasAnyActionPermission)
                            {
                                ...
                                render: function(data, type, row, meta) {
                                    ...
                                    var btnDetails = '<li><a class="dropdown-item" href="{{ url("master-data/parameter") }}/' + row.id + '/details">' +
                                        '<i class="ti ti-list-details me-2 text-info"></i>Manage Details</a></li>';

                                    var btnEdit = @json($hasUpdatePermission) ? '<li><a class="dropdown-item" href="/settings/parameter/edit/' + row.id + '">' +
                                    ...
                                    if (row.deleted_at == null) {
                                        return dropdownStart + btnDetails + btnEdit + btnDelete + dropdownEnd;
                                    } else {
                                        return dropdownStart + btnDetails + btnEdit + btnRestore + dropdownEnd;
                                    }
                                },
                            }
                            @endif
                        ...
                        buttons: [{
                            ...
                            text: '<i class="ti ti-plus me-sm-1"></i> <span class="d-none d-sm-inline-block">Add Parameter</span>',
                            className: "create-new btn btn-primary",
                            action: function(e, dt, node, config) {
                                window.location = '{{ route("parameter.insert.view") }}';
                            },
                        }, @endif ],
                    }),
                    $("div.head-label").html('<h4 class="card-title mb-0">Parameter</h4>');

                $("#btnFilter").click(function() {
                    var status = $('#selectStatus').find(':selected').val();

                    var path = "{{ url('master-data/parameter') }}?";
                ...
                $("#btnResetFilter").click(function() {
                    window.location = '{{ route("parameter.index.view") }}';
                });
                ...
                $('#deleteModal').on('show.bs.modal', function(event) {
                    ...
                    $('#parameter-id-deleted').val(id);
                    $('#parameter-name-deleted').text(name);
                });
                ...
                $('#restoreModal').on('show.bs.modal', function(event) {
                    ...
                    $('#parameter-id-restore').val(id);
                    $('#parameter-name-restore').text(name);
                });
            });
        </script>
    @endpush
