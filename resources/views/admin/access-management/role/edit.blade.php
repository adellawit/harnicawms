        ...
        <div class="row mb-3">
            ...
            <div class="col-md mb-4 mb-md-2">
                <div class="card accordion mt-3" id="accordionWithIcon">
                    <h5 class="card-header fw-bold" style="color: #212529">Edit Role</h5>
                    <form method="POST" enctype="multipart/form-data" action="{{ route('roles.edit.data') }}"
                        id="postForm">
                        @csrf
                                    ...
                                    <div class="row g-3">
                                        <div class="col-md-12 d-none">
                                            ...
                                            <input type="text" id="id" name="id" class="form-control"
                                                placeholder="Input ID" value="{{ $role->id }}" readonly />
                                        </div>
                                ...
                                <div class="accordion-body">
                                    <table class="table table-bordered" id="permissionTable">
                                        ...
                                        <tbody>
                                            ...
                                            @php
                                                $permColumns = ['create', 'read', 'update', 'delete', 'custom1', 'custom2', 'custom3', 'custom4', 'custom5'];
                                                $dbColumnMap = fn($perm) => match($perm) {
                                                    'custom1' => 'is_custom_1', 'custom2' => 'is_custom_2',
                                                    'custom3' => 'is_custom_3', 'custom4' => 'is_custom_4',
                                                    'custom5' => 'is_custom_5', default => "is_$perm"
                                                };
                                            @endphp
                                            @forelse ($parentMenus as $parent)
                                                @php $parentPerms = $rolePermissions->get($parent->id); @endphp
                                                ...
                                                <tr class="table-secondary">
                                                    ...
                                                    @foreach ($permColumns as $perm)
                                                        @php $isChecked = $parentPerms ? ($parentPerms->{$dbColumnMap($perm)} ?? false) : false; @endphp
                                                        <td class="text-center">
                                                            <input type="hidden" name="permissions[{{ $parent->id }}][{{ $perm }}]" value="0">
                                                ...
                                                @foreach ($parent->children as $child)
                                                    @php
                                                        $childPerms = $rolePermissions->get($child->id);
                                                    ...
                                                    <tr class="{{ $hasGrandChildren ? 'table-light' : '' }}">
                                                        ...
                                                        @foreach ($permColumns as $perm)
                                                            @php $isChecked = $childPerms ? ($childPerms->{$dbColumnMap($perm)} ?? false) : false; @endphp
                                                            <td class="text-center">
                                                                <input type="hidden" name="permissions[{{ $child->id }}][{{ $perm }}]" value="0">
                                                    ...
                                                    @if($hasGrandChildren)
                                                        @foreach ($child->children as $grandChild)
                                                            @php $gcPerms = $rolePermissions->get($grandChild->id); @endphp
                                                            <tr>
                                                                ...
                                                                @foreach ($permColumns as $perm)
                                                                    @php $isChecked = $gcPerms ? ($gcPerms->{$dbColumnMap($perm)} ?? false) : false; @endphp
                                                                    <td class="text-center">
                                                                        <input type="hidden" name="permissions[{{ $grandChild->id }}][{{ $perm }}]" value="0">
                                                                        <input type="checkbox" class="form-check-input child-checkbox"
