<x-app-layout>
    ...
    <div class="container-xxl flex-grow-1 container-p-y" style="padding-bottom: 70px !important;">
        ...
        <div class="card">
            <h5 class="card-header fw-bold">Add Product Type</h5>
            <form method="POST" action="{{ route('product.nature.insert.data') }}" id="postForm">
                @csrf
                        ...
                        <div class="col-md-6">
                            ...
                            <select id="parent_id" name="parent_id" class="select2 form-select" data-allow-clear="true">
                                ...
                                @foreach($natures as $nat)
                                    <option value="{{ $nat->id }}" {{ old('parent_id') == $nat->id ? 'selected' : '' }}>{{ $nat->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-6">
                            <label class="form-label" for="code">Code <span class="text-danger">*</span></label>
                            <input type="text" id="code" name="code" class="form-control" placeholder="Product type code" value="{{ old('code') }}" required />
                        </div>
                        <div class="col-md-6">
                            <label class="form-label" for="name">Name <span class="text-danger">*</span></label>
                            <input type="text" id="name" name="name" class="form-control" placeholder="Product type name" value="{{ old('name') }}" required />
                        </div>
                        <div class="col-12">
