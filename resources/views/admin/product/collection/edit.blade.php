...
<x-app-layout>
    ...
    <div class="container-xxl flex-grow-1 container-p-y" style="padding-bottom: 70px !important;">
        ...
        <div class="card">
            <h5 class="card-header fw-bold">Edit Product Collection</h5>
            <form method="POST" action="{{ route('product.collection.edit.data') }}" id="postForm">
                @csrf
                        ...
                        <div class="col-md-6">
                            ...
                            <select id="parent_id" name="parent_id" class="select2 form-select" data-allow-clear="true">
                                ...
                                @foreach($collections as $col)
                                    <option value="{{ $col->id }}" {{ old('parent_id', $collection->parent_id) == $col->id ? 'selected' : '' }}>{{ $col->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-6">
                            <label class="form-label" for="sort_order">Sort Order</label>
                            <input type="number" id="sort_order" name="sort_order" class="form-control" value="{{ old('sort_order', $collection->sort_order ?? 0) }}" min="0" />
                        </div>
                        <div class="col-md-6">
                            <label class="form-label" for="name">Name <span class="text-danger">*</span></label>
                            <input type="text" id="name" name="name" class="form-control" value="{{ old('name', $collection->name) }}" required />
                        </div>
                        <div class="col-md-6">
                            <label class="form-label" for="code">Code</label>
                            <input type="text" id="code" name="code" class="form-control" value="{{ old('code', $collection->code) }}" />
                        </div>
                        <div class="col-12">
                            <label class="form-label" for="description">Description</label>
                            <textarea id="description" name="description" class="form-control" rows="2">{{ old('description', $collection->description) }}</textarea>
                        </div>
                    </div>
                </div>
            </form>
