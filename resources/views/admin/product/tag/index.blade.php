<x-app-layout>
    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        @php
            $hasUpdatePermission = session('permissions.Tag.is_update', false) == 1;
            $hasDeletePermission = session('permissions.Tag.is_delete', false) == 1;
            $hasCreatePermission = session('permissions.Tag.is_create', false) == 1;
    ...
    <x-confirm-modal id="deleteModal" title="Delete" :action="route('product.tag.delete.data')" confirmText="Submit">
        <p>Are you sure you want to delete <strong id="tag-name-deleted"></strong>?</p>
        <input type="hidden" id="tag-id-deleted" name="tag_id_deleted" />
    </x-confirm-modal>

    <x-confirm-modal id="restoreModal" title="Restore" :action="route('product.tag.restore.data')" confirmText="Submit">
        <p>Are you sure you want to restore <strong id="tag-name-restore"></strong>?</p>
        <input type="hidden" id="tag-id-restore" name="tag_id_restored" />
    </x-confirm-modal>
            ...
            $(document).ready(function() {
                $('#table').DataTable({
                    ...
                    columns: [
                        ...
                        { data: 'type_label', orderable: true, searchable: false },
                        { data: 'color', orderable: false, searchable: false, render: function(d) { return d ? '<span class="badge" style="background-color:'+d+'">'+d+'</span>' : '-'; } },
                        ...
                        @if($hasAnyActionPermission){ data: null, orderable: false, searchable: false, render: function(d,t,r) {
                            ...
                            if (@json($hasUpdatePermission)) html += '<li><a class="dropdown-item" href="{{ url("product/tag/edit") }}/'+r.id+'"><i class="ti ti-pencil me-2 text-warning"></i>Edit</a></li>';
                ...
                $("div.head-label").html('<h4 class="card-title mb-0">Product Tags</h4>');
                $("#btnFilter").click(function() { var s=$('#selectStatus').val(); window.location = '/product/tag' + (s ? '?status='+s : ''); });
                $("#btnResetFilter").click(function() { window.location = '/product/tag'; });
                $('#deleteModal').on('show.bs.modal', function(e) { var b=$(e.relatedTarget); $('#tag-id-deleted').val(b.data('id')); $('#tag-name-deleted').text(b.data('name')); });
                $('#restoreModal').on('show.bs.modal', function(e) { var b=$(e.relatedTarget); $('#tag-id-restore').val(b.data('id')); $('#tag-name-restore').text(b.data('name')); });
            });
        </script>
    @endpush
</x-app-layout>
