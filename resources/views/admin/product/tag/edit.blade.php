@php /** @var \App\Models\ProductTag $tag */ @endphp
<x-app-layout>
    ...
    <div class="container-xxl flex-grow-1 container-p-y" style="padding-bottom: 70px !important;">
        ...
        <div class="card">
            <h5 class="card-header fw-bold">Edit Product Tag</h5>
            <form method="POST" action="{{ route('product.tag.edit.data') }}" id="postForm">
                @csrf
                ...
                <div class="card-body">
                    <div class="row g-3">
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="text" id="code" name="code" class="form-control" value="{{ old('code', $tag->code) }}" />
                        </div>
                        <div class="col-md-6">
                            <label class="form-label" for="type">Type <span class="text-danger">*</span></label>
                            <select id="type" name="type" class="form-select" required>
                                @foreach(\App\Models\ProductTag::typeOptions() as $k => $v)
                                    <option value="{{ $k }}" {{ old('type', $tag->type) == $k ? 'selected' : '' }}>{{ $v }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-6">
                            <label class="form-label" for="color">Color</label>
                            <input type="text" id="color" name="color" class="form-control" value="{{ old('color', $tag->color) }}" />
                        </div>
                        <div class="col-md-6">
                            <label class="form-label" for="sort_order">Sort Order</label>
                            <input type="number" id="sort_order" name="sort_order" class="form-control" value="{{ old('sort_order', $tag->sort_order ?? 0) }}" min="0" />
                        </div>
                        <div class="col-12">
                            <label class="form-label" for="description">Description</label>
                            <textarea id="description" name="description" class="form-control" rows="2">{{ old('description', $tag->description) }}</textarea>
                        </div>
                    </div>
                </div>
            </form>
