...
@section('content')
    <header class="shop-catalog-header">
        <div class="d-flex flex-column flex-sm-row flex-sm-wrap align-items-stretch align-items-sm-center justify-content-between gap-2 gap-sm-3">
            <div class="flex-grow-1 min-w-0">
                <h1 class="shop-page-title">Katalog</h1>
            </div>
    ...
    @if ($productTypes->isNotEmpty())
        <div class="shop-nature-scroll">
            <a href="{{ route('customer.shop', array_filter(['q' => $search])) }}"
                class="btn btn-sm btn-outline-secondary shop-nature-pill {{ ! $natureId ? 'active' : '' }}">Semua</a>
            @foreach ($productTypes as $type)
                @if ($type->products_count > 0)
                    <a href="{{ route('customer.shop', ['nature_id' => $type->id, 'q' => $search]) }}"
                        class="btn btn-sm btn-outline-secondary shop-nature-pill {{ $natureId === $type->id ? 'active' : '' }}">
                        {{ $type->name }}
                    </a>
                @endif
            @endforeach
        ...
        <div class="shop-cart-bar d-lg-none">
            <div class="d-flex align-items-center justify-content-between gap-2">
                ...
                <div class="d-flex gap-2 flex-shrink-0">
                    ...
                    <a href="{{ route('customer.checkout') }}" class="btn btn-primary btn-sm">Checkout</a>
                </div>
            </div>
        </div>
    @endif
    ...
    <div class="offcanvas offcanvas-end shop-cart-offcanvas" tabindex="-1" id="cartOffcanvas">
        ...
        <div class="offcanvas-body d-flex flex-column p-0">
            <div class="flex-grow-1 overflow-auto p-3" id="cartItemsList">
                @include('customer.shop._cart-items', ['cart' => $cart, 'summary' => $summary])
            </div>
            <div class="border-top p-3 bg-light" id="cartFooter">
                @include('customer.shop._cart-footer', ['summary' => $summary])
            </div>
        </div>
    </div>
